...
<p align="center">
	...
	<a href="https://github.com/sherifabdlnaby/kubephp/blob/main/LICENSE">
...
- [Demo applications](#running-the-demo-applications) are available for both frameworks to give you a quick start:
    - use `mise run demo:symfony:setup` for **Symfony Demo** (latest Symfony 7.x)
    - use `mise run demo:laravel:setup` for **Laravel** (latest Laravel 11.x).
...
- Copy this repository `Dockerfile`, `docker` Directory, `mise.toml`, `docker-compose.yml`, `docker-compose.prod.yml` and `.dockerignore` to your application root directory and configure it to your needs.
...
3. `mise run build && mise run up` for development setup, `mise run deploy` to run the production variant.
...
- PHP >= 8.2 Application
- [mise](https://mise.jdx.dev) for the dev tasks and linters (optional, but recommended — see below)

# Local Development with mise

This repo uses [mise](https://mise.jdx.dev) as the single source of truth for dev tasks (the docker compose wrappers) and the linters/formatters. It replaces the old Makefile.

<details>
<summary><strong>Install mise</strong></summary>

```bash
brew install mise          # macOS (or see https://mise.jdx.dev/installing-mise.html)
```

Then activate it in your shell and confirm it's healthy:
...
```bash
mise run check      # run all linters/formatters (alias: mise run lint; add --fix to autofix)
mise run up         # start the dev stack (docker compose up -d)
mise run deploy     # run the production image
...
2. Copy `Dockerfile`, `docker` Directory, `mise.toml`, `docker-compose.yml`, `docker-compose.prod.yml` and `.dockerignore` Into your Application Repository.
...
2. Run `mise run up` for development or `mise run deploy` for production.
    1. For Dev: `mise run up` is just an alias for `docker compose up -d`
...
- The image comes with handy _mise_ tasks to build the image using Docker-Compose files, it's handy when manually building the image for development or in a not-orchestrated docker host.
...
### PHP Configuration
1. PHP `base` Configuration that are common in all environments in `docker/php/base-php.ini`[🔗](https://github.com/sherifabdlnaby/kubephp/blob/main/docker/php/base-php.ini)
1. PHP `prod` Only Configuration  `docker/php/php-prod.ini`[🔗](https://github.com/sherifabdlnaby/kubephp/blob/main/docker/php/prod-php.ini)
2. PHP `dev` Only Configuration  `docker/php/php-dev.ini`[🔗](https://github.com/sherifabdlnaby/kubephp/blob/main/docker/php/dev-php.ini)

### PHP FPM Configuration
1. PHP FPM Configuration  `docker/fpm/*.conf` [🔗](https://github.com/sherifabdlnaby/kubephp/blob/main/docker/fpm)

### Nginx Configuration
1. Nginx Configuration  `docker/nginx/*.conf && docker/nginx/conf.d/* ` [🔗](https://github.com/sherifabdlnaby/kubephp/blob/main/docker/nginx)
...
```bash
mise run demo:symfony:setup  # Set up the demo
mise run demo:symfony:up     # Start in dev mode
...
mise run demo:symfony:up

# Start in production mode (optimized)
mise run demo:symfony:deploy
```
...
```bash
mise run demo:laravel:setup  # Set up the demo
mise run demo:laravel:up     # Start in dev mode
# Visit http://localhost:8080
```
...
```bash
# Start in development mode (with hot-reloading)
mise run demo:laravel:up

# Start in production mode (optimized)
mise run demo:laravel:deploy
```
...
mise run command -- php artisan migrate        # Laravel
mise run command -- php bin/console cache:clear # Symfony
...
[MIT License](https://github.com/sherifabdlnaby/kubephp/blob/main/LICENSE)
